...
predictor = pdx.deploy.Predictor('../P0008-T0014_export_model/inference_model/inference_model', use_gpu=True)
...
im=cv2.imread('./2022-04-28new/380_color.png')
depth=np.load('./2022-04-28new/380depth_image_real.npy')
rect_center,acceptedRects=od.obs_detection_file('./2022-04-28new/380depth_image_real.npy')
...
for i in range(len(rect_center)):
    ...
    cv2.putText(result_vis,str(obs.angle)+"degree",(int(x),int(y)+20),cv2.FONT_HERSHEY_SIMPLEX,0.5,(0,0,255),1)
    #显示宽度
    cv2.putText(result_vis,"width:"+str(obs.width)+'m',(int(x),int(y)+40),cv2.FONT_HERSHEY_SIMPLEX,0.5,(0,0,255),1)
